...
module.exports = {
    ...
    command:function(builder, cb) {
        ...
        // build command:  ./cjs steal/buildjs OpsPortal opstools/WordpressPlugin
        ...
        async.series([
            ...
            function (next) {
                ...
                AD.log('<green>building</green> opstools/WordpressPlugin JS files');

                // Minify js/ejs files
                transform({
                    main: path.join('opstools', 'WordpressPlugin', 'WordpressPlugin'),
                    config: "stealconfig.js"
                }, {
                        minify: true,
                        ignore: [
                            /^.*(.css)+/, // Ignore css files
                            /^(?!opstools\/WordpressPlugin.*)/, // Ignore all are not plugin scripts
                        ]
                    }).then(function (transform) {
                        // Get the main module and it's dependencies as a string
                        var main = transform();

                        fs.writeFile(path.join('opstools', 'WordpressPlugin', 'production.js'), main.code, "utf8", function (err) {
                            if (err) {
                                AD.log.error('<red>could not write minified opstools/WordpressPlugin JS file !</red>');
                                next(err);
                            }
                    ...
                    .catch(function (err) {
                        AD.log.error('<red>could not complete opstools/WordpressPlugin JS build!</red>', err);
                        next(err);
                    });
            },
            ...
            function (next) {
                AD.log('<green>building</green> opstools/WordpressPlugin CSS files');
                ...
                transform({
                    ...
                    config: "stealconfig.js"
                        ...
                        fs.writeFile(path.join('opstools', 'WordpressPlugin', 'production.css'), main.code, "utf8", function (err) {
                            if (err) {
                                AD.log.error('<red>could not write minified opstools/WordpressPlugin CSS file !</red>');
                                next(err);
                            }
                    ...
                    .catch(function (err) {
                        AD.log.error('<red>could not complete opstools/WordpressPlugin CSS build!</red>', err);
                        next(err);
                    });
            },
            ...
            function(next) {
                ...
                var patches = [
                    { file:path.join('opstools', 'WordpressPlugin', 'production.js'), tag:'packages/OpsPortal-WordpressPlugin.js', replace:'OpsPortal/production.js'}
                ];
